    ...
    struct localFns {
                                      ...
                                      size_t                     expectedNum)
        {
            ...
            for (size_t i = 0; i < expectedNum; i++) {
              ASSERT_EQ(
                  events.timedPopFront(&buffer[i], bdlt::CurrentTime::now() +
                                                       bsls::TimeInterval(0.1)),
                  0);
            }
        ...
        static void checkNoEvents(bdlcc::Deque<bsl::string>& events)
        {
            ...
            ASSERT_NE(
                events.timedPopFront(&emptyStr, bdlt::CurrentTime::now() +
                                                    bsls::TimeInterval(0.01)),
                0);
        }

        static bool
        waitForWriteCall(const bsl::deque<mwcio::TestChannel::WriteCall> &calls,
                         const bsls::TimeInterval &timeout) {
